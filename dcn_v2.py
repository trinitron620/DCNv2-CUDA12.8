...
class DCNv2Layer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, deformable_groups=1):
        ...
        self.offset = nn.Parameter(torch.zeros(1, deformable_groups * 2 * kernel_size * kernel_size, 1, 1))
        self.mask = nn.Parameter(torch.ones(1, deformable_groups * kernel_size * kernel_size, 1, 1))
        self.stride = stride
    ...
    def forward(self, x):
        offset = self.offset.expand(x.size(0), -1, x.size(2), x.size(3))
        mask = self.mask.expand(x.size(0), -1, x.size(2), x.size(3))
...
class _DCNv2(Function):
    ...
    def forward(
        ...
        input = input.to(torch.float32).contiguous()
